   ...
   FROM
     (SELECT movie_id
      ...
      WHERE k.keyword IN ('aging-out',
                    ...
                    'shackleford',
                    'townhouse',
                    'village-vanguard-manhattan-new-york-city',
                    'winter-in-new-york-city')) AS mk
   JOIN
     (SELECT movie_id
      FROM movie_companies mc
      JOIN company_name cn ON mc.company_id = cn.id
      JOIN company_type ct ON mc.company_type_id = ct.id
      WHERE cn.country_code IN ('[is]',
                          '[mx]',
                          '[ro]',
                          '[ve]')
     ...
     (SELECT movie_id
      ...
      WHERE n.gender IN ('m')
        AND rt.role IN ('cinematographer',
                  'miscellaneous crew')) AS ci ON mk.movie_id = ci.movie_id)
SELECT COUNT(*)
FROM title AS t
