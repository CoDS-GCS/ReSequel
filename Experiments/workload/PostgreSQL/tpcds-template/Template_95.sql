...
WHERE ss1.d_qoy = ###
  ...
  AND ws3.d_year = ###
  AND CASE
          WHEN ws1.web_sales > ### THEN (ws2.web_sales*^^^)/ws1.web_sales
          ELSE NULL
      END > CASE
                WHEN ss1.store_sales > ### THEN (ss2.store_sales*^^^)/ss1.store_sales
                ELSE NULL
            END
  AND CASE
          WHEN ws2.web_sales > ### THEN (ws3.web_sales*^^^)/ws2.web_sales
          ELSE NULL
      END > CASE
                WHEN ss2.store_sales > ### THEN (ss3.store_sales*^^^)/ss2.store_sales
                ELSE NULL
            END
