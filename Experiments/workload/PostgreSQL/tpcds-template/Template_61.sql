...
FROM
  (SELECT 'web' AS channel,
   ...
   WHERE (web.return_rank <= ###
          OR web.currency_rank <= ###)
   ...
   WHERE (catalog.return_rank <= ###
          OR catalog.currency_rank <=###)
   ...
   WHERE (store.return_rank <= ###
          OR store.currency_rank <= ###)) sq1
ORDER BY ###,
         ###,
         ###,
         ###
